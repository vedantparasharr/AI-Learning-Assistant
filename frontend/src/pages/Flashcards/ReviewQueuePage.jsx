...
import { PageShell, PrimaryButton, InlineLinkButton } from "../../components/common/ui";
...
export default function ReviewQueuePage() {
  ...
  const { data: fetchedQueue, isPending: loading, isError, refetch: loadQueue } = useQuery({
    queryKey: ['reviewQueue', topicKey],
    queryFn: async () => {
      const res = await flashcardService.getQueue(topicKey);
      return res.data || [];
    },
    // We handle the local queue manually by shifting items off, so we don't want 
    // automatic background refetches to overwrite our local state unexpectedly.
  ...
  useEffect(() => {
    if (fetchedQueue) {
      ...
      setInitialCount(fetchedQueue.length);
      setReviewedCount(0);
      setAgainCount(0);
      setRevealed(false);
  ...
  const reviewMutation = useMutation({
    mutationFn: ({ cardId, rating }) => flashcardService.reviewCard(cardId, rating),
    onError: (err) => {
  ...
  const handleReview = useCallback(async (rating) => {
    ...
    reviewMutation.mutate({ cardId: cardToReview._id, rating });

    // 4. Reset flipping state after animation completes (matches CSS transition)
    setTimeout(() => {
      setIsFlipping(false);
    }, 600);
  }, [currentCard, isFlipping, reviewMutation]);
      ...
      <div className="max-w-container-max mx-auto pb-lg animate-pulse">
        ...
        >
          <div className="flex flex-col items-center justify-center py-8">
            <div className="w-full max-w-[800px] bg-surface-container-lowest rounded-xl border border-outline-variant/60 min-h-[400px] p-12 flex flex-col justify-center items-center text-center">
      ...
      <PageShell
        ...
        actions={
          <div className="flex items-center gap-4 text-body-sm text-on-surface-variant bg-surface-container-lowest border border-outline-variant/60 rounded-xl px-4 py-2 shadow-sm h-11">
            ...
            <div className="flex-1 h-[4px] bg-surface-container-high rounded-full overflow-hidden min-w-[60px] max-w-[120px]">
              <div
        ...
        <div className="flex flex-col items-center justify-center py-8">
          {!currentCard ? (
            <div className="w-full max-w-md text-center">
              <div className="bg-surface-container-lowest rounded-xl p-8 border border-outline-variant/60 shadow-sm">
                ...
                <div className="flex flex-col gap-3">
                  <PrimaryButton
                    onClick={loadQueue}
                    className="w-full h-12 text-label-md uppercase tracking-wider"
                  >
                  ...
                  <InlineLinkButton
                    to="/dashboard"
                    className="w-full text-label-md uppercase tracking-wider"
                  >
                    Back to Dashboard
                  </InlineLinkButton>
                </div>
            ...
            <div className="w-full max-w-[800px]">
              ...
              <div className="flip-card mx-auto min-h-[400px] w-full">
                <div className={`flip-card-inner ${revealed ? "flipped" : ""} min-h-[400px]`}>
                  ...
                  <button
                    ...
                    className="flip-card-front bg-surface-container-lowest rounded-xl border border-outline-variant/60 hover:border-primary/40 focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2 transition-all p-12 flex flex-col justify-center text-center cursor-pointer min-h-[400px] w-full"
                  >
                  ...
                  {/* BACK */}
                  <div className="flip-card-back bg-surface-container-lowest rounded-xl border border-outline-variant/60 p-12 flex flex-col justify-center text-center min-h-[400px] w-full">
              ...
              <div className="w-full min-h-[96px] relative flex items-center justify-center mt-8">
                {/* SHOW ANSWER BUTTON */}
                <div
                  className={`w-full max-w-xs transition-all duration-300 transform ${revealed
                ...
                >
                  <PrimaryButton
                    ...
                    className="w-full h-12 text-label-md uppercase tracking-wider shadow-sm"
                  >
                ...
                <div
                  className={`w-full transition-all duration-300 transform ${revealed
                      ? "opacity-100 scale-100 relative"
                      : "opacity-0 scale-95 pointer-events-none absolute"
                    }`}
                >
